...
test("inspection API enforces provider ownership and customer-token read", async () => {
  ...
  // Replacing a checklist is atomic: a failed insert cannot erase the previous version.
  assert.match(source, /db\.batch\(\[deleteExisting, db\.insert\(jobInspectionItems\)\.values\(rows\)\]/);
...
test("job_inspection_items migration is additive (new table only)", async () => {
  const migration = await read("drizzle/0056_job_inspection_items.sql");
