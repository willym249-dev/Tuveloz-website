...
- compares each rendered PDF body and HTML body with a **bidirectional, order-sensitive NFKD-normalised token edit distance**, failing above 2% or on a contiguous one-sided gap longer than one token; directly scans extracted PDF text for any banned phrase not present in the vetted source; and fails outright if extraction is impossible;
