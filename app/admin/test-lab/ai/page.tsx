      ...
      <div style={{ margin: "0 auto", maxWidth: "64rem" }}>
        ...
        <section style={{ ...cardStyle, borderColor: enabled ? "#ff6a00" : "#9a5a32", marginBottom: "1rem" }}>
          <strong style={{ color: "#ff9a55" }}>
            {enabled ? "AI TEST IS AVAILABLE" : "AI TEST IS SAFELY UNAVAILABLE"}
          </strong>
          <p style={{ lineHeight: 1.6, marginBottom: 0 }}>
            {enabled
              ? "Drafting runs on the same GPT/Gemini/Claude council as the public assistant. Every result remains a draft and requires owner review. No result is saved or submitted to the marketplace."
              : "The code is installed, but no AI request will run until at least one provider key (OPENAI_API_KEY, GEMINI_API_KEY, or ANTHROPIC_API_KEY) is stored with wrangler secret put."}
          </p>
        </section>
