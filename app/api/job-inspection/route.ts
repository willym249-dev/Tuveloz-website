...
export async function POST(request: Request) {
  ...
  // Replace-all: the provider always sends the full current checklist.
  await db.delete(jobInspectionItems).where(eq(jobInspectionItems.requestId, job.requestId));
  if (rows.length) await db.insert(jobInspectionItems).values(rows);
