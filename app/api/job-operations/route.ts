...
async function invalidateStaleCheckoutBeforeScopeChange(input: {
  ...
  for (const payment of active) {
    ...
    const auditInsert = db.insert(jobLifecycleEvents).select(db.select({
      ...
      requestId: sql<string>`${input.requestId}`.as("requestId"),
      quoteId: sql<string>`${input.quoteId}`.as("quoteId"),
      providerId: sql<string>`${input.providerId}`.as("providerId"),
      actorRole: sql<string>`system`.as("actorRole"),
      actorId: sql<string>`stripe-checkout-scope-guard`.as("actorId"),
      eventType: sql<string>`${eventType}`.as("eventType"),
      fromStatus: sql<string>`${input.requestStatus}`.as("fromStatus"),
      toStatus: sql<string>`${input.requestStatus}`.as("toStatus"),
      scopeVersion: sql<number>`${input.currentScopeVersion}`.as("scopeVersion"),
      authorizationSnapshotId: sql<string>``.as("authorizationSnapshotId"),
      reasonCode: sql<string>`${reasonCode}`.as("reasonCode"),
      details: sql<string>`${eventDetails}`.as("details"),
      previousEventHash: sql<string>`${previousEventHash}`.as("previousEventHash"),
      eventHash: sql<string>`${eventHash}`.as("eventHash"),
      occurredAt: sql<string>`${input.changedAt}`.as("occurredAt"),
      createdAt: sql<string>`${input.changedAt}`.as("createdAt"),
    }).from(stripePayments).where(and(
      eq(stripePayments.id, payment.id),
...
export async function POST(request: Request) {
  ...
  if (action === "authorize-change-order") {
    ...
    const acceptanceInsert = db.insert(customerAgreementAcceptances).select(
      db.select({
        id: sql<string>`${acceptanceId}`.as("id"),
        customerEmail: sql<string>`${actor.email}`.as("customerEmail"),
        requestId: sql<string>`${requestId}`.as("requestId"),
        quoteId: sql<string>`${context.quoteId}`.as("quoteId"),
        scopeVersion: sql<number>`${order.proposedScopeVersion}`.as("scopeVersion"),
        scopeSnapshot: sql<string>`${scopeSnapshot}`.as("scopeSnapshot"),
        agreementKey: sql<string>`change_order_authorization`.as("agreementKey"),
        agreementVersion: sql<string>`${JOB_OPERATIONS_RULES_VERSION}`.as("agreementVersion"),
        agreementHash: sql<string>`${agreementHash}`.as("agreementHash"),
        agreementText: sql<string>`${agreementText}`.as("agreementText"),
        acceptedByName: sql<string>`${acceptedByName}`.as("acceptedByName"),
        acceptanceAction: sql<string>`affirmative-change-order-authorization-checkbox`.as("acceptanceAction"),
        acceptedAt: sql<string>`${now}`.as("acceptedAt"),
        ipAddress: sql<string>`${requestIp(request)}`.as("ipAddress"),
        sessionId: sql<string>`${actor.sessionId}`.as("sessionId"),
        deviceContext: sql<string>`${deviceContext}`.as("deviceContext"),
    ...
    const scopeInsert = db.insert(jobScopeVersions).select(
      db.select({
        id: sql<string>`${scopeVersionId}`.as("id"),
        requestId: sql<string>`${requestId}`.as("requestId"),
        quoteId: sql<string>`${context.quoteId}`.as("quoteId"),
        version: sql<number>`${order.proposedScopeVersion}`.as("version"),
        serviceCodes: sql<string>`${JSON.stringify(serviceCodes)}`.as("serviceCodes"),
        jurisdiction: sql<string>`${context.jurisdiction}`.as("jurisdiction"),
        scheduledFor: sql<string>`${scheduledFor}`.as("scheduledFor"),
        scopeDetails: sql<string>`${order.scopeDetails}`.as("scopeDetails"),
        priceBreakdown: sql<string>`${order.priceBreakdown}`.as("priceBreakdown"),
        partsResponsibility: sql<string>`${clean(scopeDetails.partsResponsibility, 120)}`.as("partsResponsibility"),
        changeReason: sql<string>`${order.reason}`.as("changeReason"),
        createdByProviderId: sql<string>`${context.providerId}`.as("createdByProviderId"),
        createdByPersonId: sql<string>`${context.personId}`.as("createdByPersonId"),
        providerApprovedAt: sql<string>`${order.providerApprovedAt}`.as("providerApprovedAt"),
        customerAuthorizedAt: sql<string>`${now}`.as("customerAuthorizedAt"),
        authorizationDecisionId: sql<string>`${stageDecision.result.decisionId}`.as("authorizationDecisionId"),
        supersedesScopeVersionId: sql<string>`${priorScope.id}`.as("supersedesScopeVersionId"),
        createdAt: sql<string>`${now}`.as("createdAt"),
      }).from(jobChangeOrders).where(authorizationPrerequisite).limit(1),
