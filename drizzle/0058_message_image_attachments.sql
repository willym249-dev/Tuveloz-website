...
ALTER TABLE `job_messages` ADD `scan_attempted_at` text DEFAULT '' NOT NULL;
ALTER TABLE `job_messages` ADD `scan_attempt_count` integer DEFAULT 0 NOT NULL;
CREATE INDEX `job_messages_scan_status_idx` ON `job_messages` (`scan_status`);
CREATE INDEX `job_messages_scan_attempted_idx` ON `job_messages` (`scan_status`, `scan_attempted_at`);
