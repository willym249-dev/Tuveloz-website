...
BEGIN
	...
	) THEN 1 ELSE RAISE(ABORT, 'identity verification insert requires the latest independent owner-operator binding') END;
END;
--> statement-breakpoint
...
BEGIN
	...
	) THEN 1 ELSE RAISE(ABORT, 'approved identity session requires exact latest application, pathway, and personnel binding') END;
END;
--> statement-breakpoint
...
BEGIN
	...
	SELECT CASE WHEN changes() = 1 THEN 1 ELSE RAISE(ABORT, 'approved identity session could not stamp exactly one personnel record') END;
	...
	SELECT CASE WHEN changes() = 1 THEN 1 ELSE RAISE(ABORT, 'approved identity session could not bind exactly one independent profile') END;
END;
--> statement-breakpoint
...
BEGIN
	...
	) THEN 1 ELSE RAISE(ABORT, 'stripe identity personnel evidence requires an approved bound session') END;
END;
--> statement-breakpoint
