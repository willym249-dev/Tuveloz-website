...
| Owner-operated ClamAV | Malware and file-safety scanning before uploaded evidence can leave quarantine | Selected in production. The scheduled runner and authenticated claim path work; the first complete file canary is still missing. |
...
September 25 the owner task reported current definitions and a successful
authenticated production claim with an empty queue. That is connection proof,
not a complete file scan. Stripe Identity is configured with
...
- `evidence_file_security_and_scanner` — still needs a real permitted file to
  complete the production scan, plus capacity and security review
