...
| 2027-06-22 | Reconfirm tuveloz.com renewal and the registrar payment method before its July 22, 2027 expiration. September 26 signed-in Porkbun review confirmed auto-renew on, privacy service selected, and a saved Link via Stripe payment method. No card expiry was shown and no charge was tested; saved billing does not guarantee a future successful payment. Internal reminder checkpoint | hello@tuveloz.com | open |
| 2026-10-02 | Confirm whether the inspected Porkbun contact remains accurate and reachable, or authorize a specific replacement. Its populated address/email differ from the verified business mailbox; that can be valid for an owner contact. Renewal/privacy/billing inspection is complete. Review only the remaining record-specific legal/tax/bank items in operations/business-address-review.md. Stripe support, mailbox activation, Google Payments, and public-profile corrections have dated evidence; do not repeat them or presume other fields are wrong | hello@tuveloz.com | open |
| 2026-09-27 | First automatic nightly backup verified September 26: scheduled-backup-1790413652000 completed all seven steps at 09:07:38 UTC, with D1 export, both file copies, manifest, and retention processing. Manual backup and isolated data recovery also passed. No paid upgrade | hello@tuveloz.com | done |
| 2026-10-02 | Isolated data recovery passed September 26: separate D1 restored 78 tables/355 records/383 schema objects; all table counts matched, quick check passed, foreign-key violations zero. Both expected R2 objects restored with exact paths/hashes/metadata. Actual application routes passed local recovered-data smoke checks with no outbound calls or writes. No hosted application cutover | hello@tuveloz.com | done |
| 2026-10-02 | Review cleanup of isolated recovery copies after evidence is retained privately. Includes the separate D1/R2 recovery resources and one extra unreferenced 605-byte fixture. Do not delete production data, the backup bucket, or test copies without the applicable owner approval | hello@tuveloz.com | open |
| 2026-10-18 | Renew the Tuveloz private database backups D1 Edit credential before its October 25 expiration and verify a successful scheduled backup afterward. Account-level Edit was expressly approved September 26 after Read failed. Review the existing release credential's October 26 expiration as well; do not broaden either credential silently | hello@tuveloz.com | open |
...
| 2026-10-05 | Workspace sender repaired 2026-09-04: root Google SPF include and 2048-bit Google DKIM published; an owner-authorized message reached a separate Gmail inbox with SPF, DKIM and DMARC passing. See the email-authentication runbook; DMARC enforcement still needs report review | hello@tuveloz.com | done |
...
| 2026-08-25 | Complete the [launch review packet](business/launch-gate-briefing.md) and record supported decisions in `/admin/launch-readiness`. September 5 live refresh found all 18 review controls Pending (17 required gates and one optional lane). September 26 corrected the scanner handoff: manual/automatic synthetic file results already passed, live rows still match, and current application proof validation passes. A genuine provider Identity result and required business/reviewer evidence remain incomplete; scanner-proof freshness is tracked separately | hello@tuveloz.com | blocked |
