...
| --- | --- | --- | --- |
| 2026-09-27 | Confirm the first automatic nightly backup. September 26: PR #231 merged and activation run 36227836343 succeeded; Cloudflare confirms the next run at 09:07 UTC. The first manual database/file backup and isolated local recovery already passed. No paid upgrade. This is an internal verification checkpoint, not an external deadline | hello@tuveloz.com | open |
| 2026-10-02 | Finish isolated Cloudflare recovery. September 26: separate D1 restored 78 tables/355 records/383 schema objects; all table counts matched, quick check passed, foreign-key violations zero. Private recovery R2 bucket is empty: upload needs the owner to enable the ChatGPT extension's Allow access to file URLs setting. File paths/hashes/metadata and isolated application behavior remain unverified. No production cutover. Internal review target | hello@tuveloz.com | open |
